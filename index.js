...
client.once("ready", async () => {
    ...
    // Set permissions for the command in all guilds the bot is a member of
    ...
    for (const [guildId] of guilds) {
        ...
        // Create the permissions array for the allowed roles
        ...
        try {
            ...
            console.log(
                `Permissions set for /party command in guild: ${guild.name}`,
            );
        } catch (error) {
            console.error(
                `Failed to set permissions for guild ${guild.name}:`,
                error,
            );
        }
    }
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (commandName === "party") {
        ...
        if (channel) {
            ...
            interaction.reply({
                content:
                    "Target channel not found. Please ensure the TARGET_CHANNEL_ID secret is correct.",
                ephemeral: true,
            });
        }
    }
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (action === "party" && activeParties[partyId]) {
        ...
        await interaction.user.send(
            `Here's your private server link for the **${party.duelType}** party hosted by **${party.host}**: ${joinUrl}`,
        );
